...
f_(u) &= \int_{-\infty}^{\infty} \frac{|v|}{\pi^2 (1 + (uv)^2)(1+v^2)} = \frac{1}{\pi^2} \int_0^{\infty} \frac{1}{\pi^2 (1 + u^2 s)(1+s )ds }

\end{align}
```
Sir Dhav recognized that the integrand has a partial fraction decomposition:
...
which is definitely not a Cauchy distribution. By the way, this is one of the weirdest densities I have ever seen, $f(u) \to \infty$ as $u \to 0$ and $f$ is indeterminate at $u = \pm 1$ (the limit may be evaluated using L'hospital's rule). Here is a plot comparing the Cauchy density (red) to its ratio (blue). Strangely enough, this plot shows seems to show that the ratio density has *lighter* tails, which is not in line with the simulation. Hm?
